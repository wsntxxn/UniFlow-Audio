# nohup bash ./bash_scripts/train_tts_libritts_2gpus.sh  > logs/train_tts_libritts_2gpus.log 2>&1  &
echo "Current time: $(TZ='Asia/Shanghai' date '+%Y-%m-%d %H:%M:%S')" 
echo "Running script name: $(basename "$0")" 
accelerate launch --config_file configs/accelerate/2gpus.yaml train.py \
    epoch_length=1000 \
    epochs=200 \
    ...
    trainer.wandb_config.name=librits_wo_conv_xvector_2gpus \
    exp_dir=experiments/tts/librits/wo_conv_xvector_2gpus \
    # +trainer.resume_from_checkpoint=experiments/tts/librits/wo_conv_xvector_2gpus/checkpoints/epoch_33
