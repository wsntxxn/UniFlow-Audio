# nohup bash ./bash_scripts/train_tts_libritts_8gpus_ada.sh  > logs/train_tts_libritts_8gpus_ada.log 2>&1  &
...
accelerate launch --config_file configs/accelerate/8gpus.yaml train.py \
    model.backbone.time_fusion=ada \
    data@data_dict=tts \
    epoch_length=1000 \
    epochs=200 \
    ...
    trainer.wandb_config.name=librits_wo_conv_xvector_8gpus_ada \
    exp_dir=experiments/tts/librits/wo_conv_xvector_8gpus_ada 
