...
class AudioGenerationTrainer(Trainer):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print(f'val loss_dict: {loss_dict},device:{self.accelerator.device}')
        self.val_batch_num += 1

    def get_val_metrics(self,metric_name:str):
        assert metric_name in self.val_loss_dict.keys()
        return {metric_name: self.val_loss_dict[metric_name] / self.val_batch_num}

    def on_train_epoch_end(self):
        print(f"on start of train epoch end func , train_loss: {self.train_loss},train_batch_num: {self.train_batch_num},device:{self.accelerator.device}")
