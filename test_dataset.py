...
def read_jsonl_to_mapping(
    jsonl_file: str | Path, key_col: str, value_col: str, value_base_path: str|None = None
) -> dict[str, str]:
    """
    ...
    with open(jsonl_file, 'r') as file:
        for line in file.readlines(): 
            ...
            if value_base_path is not None:
                value = Path(value_base_path) / value
                value = str(value)
            mapping[key] = value
    return mapping
...
class HDF5DatasetMixin:
    ...
    def __post_init__(self) -> None:
        #注意不能直接在类属性定义，因为可变类型可能导致所有实例共享一个字典
...
# dataclass可以构造__init__函数，接受参数并自动初始化下面这些参数
...
class AudioGenerationDataset(AudioWaveformDataset):
    ...
    def __post_init__(self, ):
        ...
        self.id_to_content = read_jsonl_to_mapping(
            self.content, id_col_in_content, self.content_col,self.base_content_path
        )
        ...
        if self.audio:
            self.id_to_audio = read_jsonl_to_mapping(
                self.audio, id_col_in_audio, self.audio_col,self.base_audio_path
            )
        else:
        ...
        if self.condition:
            ...
            self.id_to_condition = read_jsonl_to_mapping(
                self.condition, id_col_in_condition, self.condition_col,self.base_condition_path
            )
        else:
...
# get_item调用load_content_waveform，后者可能调用load_content
    def __getitem__(self, index) -> dict[str, Any]:
        audio_id = self.audio_ids[index]
        #对于tts，content是phoneme，waveform是音频，duration是phoneme_duration
...
class TextToAudioDataset(AudioGenerationDataset):
    ...
    def load_duration(self, content: Any,
                      waveform: torch.Tensor) -> Sequence[float]:
        return [10.0]  # dummy duration sequence for batchify
...
class TextToSpeechDataset(AudioGenerationDataset):
    ...
    #     self.phoneme_to_id = read_jsonl_to_mapping(
    #         self.content, "phoneme", "phoneme_id"
    #     )
    #     self.phoneme_id_to_phoneme = {
    #         v: k for k, v in self.phoneme_to_id.items()
    #     }
    @property
    def task(self):
    ...
    def load_content_waveform(self, audio_id: str) -> tuple[Any, torch.Tensor]:
        content_or_path = self.id_to_content[audio_id]
        # print(f'audio_id: {audio_id}, content_or_path: {content_or_path}') 
        # [()] 代表“读取整个数据”，将其转换为 NumPy 数组（或标量）
        with File(content_or_path, "r") as hf:
            phoneme = hf["phoneme"][audio_id][()]
            phoneme_duration = hf["phoneme_duration"][audio_id][()]
            #检测"spkid" 是否存在
            if "spkid" in hf.keys():
                spkid = hf["spkid"][audio_id][()]
                if isinstance(spkid, bytes):
                    spkid=np.int32(spkid.decode())
                    # 转换为numpy对象
            ...
            #     spkid = hf["spkid"][audio_id][()]
            #     #检查spkid是数组还是一个标量
            #     if np.isscalar(spkid):
            #         spkid = [spkid] * len(phoneme)
            #     # 如果是binary类型，转换为整数
            #     if isinstance(spkid[0], bytes):
            #         spkid=np.array([int(spk.decode()) for spk in spkid])
            # else:
            #     spkid=[0]*len(phoneme)
            if "xvector" in hf.keys():
                xvector = hf["xvector"][audio_id][()]
            else:
                xvector = None
...
if __name__ == '__main__':
    ...
    dataset = AudioGenConcatDataset(
        datasets=[
            # TextToAudioDataset(
            #     content="./data/audiocaps/train/caption.jsonl",
            #     audio="./data/audiocaps/train/audio.jsonl",
            #     base_audio_path="/cpfs_shared/jiahao.mei/data/tta",
            #     target_sr=24000
            # ),
            # PopCsSingingDataset(
            ...
            # ),
            # TextToSpeechDataset(
            #     content="./data/ljspeech/train/phoneme.jsonl",
            #     audio="./data/ljspeech/train/audio.jsonl",
            #     target_sr=16000
            # ),
            TextToSpeechDataset(
    ...
    #     dataset, collate_fn=collate_fn, batch_size=16
    # )
    dataloader = StatefulDataLoader(dataset,batch_size=14,shuffle=True,num_workers=8,collate_fn=collate_fn)
    ...
    for batch in tqdm(dataloader):
        print(batch)
        break
        # if "text_to_audio" in batch['task']:
        #     break
