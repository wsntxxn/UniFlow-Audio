...
class FastSpeech2PhonemeEncoder(FastSpeech2EncoderBase):
    def __init__(
        ...
        self.phone_embed = nn.Embedding(phone_vocab_size, d_model)
        self.pitch_embed = nn.Embedding(300, d_model)
        #	padding_idx=0 让 索引 0 对应的嵌入始终是零向量，这样在计算损失时不会影响梯度传播。
        ...
        if self.use_xvector:
            self.spk_embed = nn.Linear(xvector_dim, d_model)
        else:
            #	padding_idx=0 让 索引 0 对应的嵌入始终是零向量，这样在计算损失时不会影响梯度传播。
            self.spk_embed = nn.Embedding(n_spks, d_model,padding_idx=0)
            

    def forward(self, phoneme: torch.Tensor,spkid:torch.Tensor, lengths: Sequence[int],xvector:torch.Tensor):
        # phone_emb = self.phone_embed(phoneme)  # (B, L, d_model)
        ...
        #x:[1,L,d_model] spk_emb:[1,d_model] 广播相加
        if self.use_xvector:
            spk_emb = self.spk_embed(xvector)
            x=x+spk_emb
            
        elif self.n_spks>1 :
            #检查spkid是否小于nspks，否则警告
            if spkid>=self.n_spks:
                print("spkid should range from 0 to n_spks-1")
            else:
                spk_emb = self.spk_embed(spkid)
                x=x+spk_emb
