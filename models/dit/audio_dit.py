...
class SelectiveLayerFusionAudioDiT(UDiT):
    def __init__(
        self,
        img_size=224,
        patch_size=16,
        ...
        embed_dim=768,
        depth=12,
        fusion_layers: list[int] | str = "all",
        num_heads=12,
        mlp_ratio=4,
        qkv_bias=False,
        qk_scale=None,
        ...
        for block_idx in range(depth // 2, depth):
            if block_idx in fusion_layers:
                out_blocks.append(
                    LayerFusionDiTBlock(
                        dim=embed_dim,
    ...
    def forward(
        ...
        skips = []
        for blk_idx, blk in enumerate(self.in_blocks):
            if blk_idx in self.fusion_layers:
                x = blk(
                    x=x,
                    time_aligned_context=time_aligned_context,
                    time_token=time_token,
        ...
        for blk_idx_, blk in enumerate(self.out_blocks):
            blk_idx = blk_idx_ + self.depth // 2
            if self.use_skip:
                skip = skips.pop()
                if controlnet_skips:
                    # add to skip like u-net controlnet
