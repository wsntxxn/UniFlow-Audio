...
def contains_nan(data):
    ...
    elif isinstance(data, float):  # 新增原生 float 检测
...
class Trainer(CheckpointMixin):
    ...
    def val_loop(self) -> None:
        print(f" val loop start ,device:{self.accelerator.device}")
        self.model.eval()
        ...
        print(f" val loop end,device:{self.accelerator.device}")
    ...
    def train_loop(self) -> None:
        print(f"train loop start,device:{self.accelerator.device}")
        torch.set_grad_enabled(True)
        self.model.train()
        ...
        for batch_idx in range_iterator:
            try:
                ...
                batch = next(self.train_data_iterator)
            #检查nan数据
            nan_flag = torch.tensor(1 if contains_nan(batch) else 0, device=self.accelerator.device)
            ...
            if nan_flag.item() == 1:
                if contains_nan(batch):
                    ...
                    print(f"\nStep={self.step},device={self.accelerator.device} NaN batch,样本: {lis}")
                self.step += 1
                #TODO 使用梯度累积时，此处清空梯度可能出问题
            ...
            # if contains_nan(batch):
            #     lis=check_nan_in_batch(batch)
            #     print(f"[Step {self.step}] 检测到 NaN，跳过该 batch")
            #     print(f"nan 数据:{lis}")
            #     self.step += 1
            #     continue  
        ...
        if self.metric_monitor is not None:
            ...
            metric_dict: dict = self.get_val_metrics(self.metric_monitor.metric_name)
        ...
        print(f"train loop end,device:{self.accelerator.device}")
        self.accelerator.wait_for_everyone()
