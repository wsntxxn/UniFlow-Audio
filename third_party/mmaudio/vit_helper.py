    ...
    def forward(
        ...
        **einops_dims
        ...
        if tok_mask is not None:
            ...
            tok_mask = tok_mask.unsqueeze(1).expand(-1, h, -1).reshape(
                -1, tok_mask.shape[1]
            )
        ...
        (cls_q,
         q_), (cls_k,
               k_), (cls_v,
                     v_) = map(lambda t: (t[:, 0:1], t[:, 1:]), (q, k, v))
        ...
        if tok_mask is not None:
            ...
            mask_ = rearrange(
                mask_, f'{einops_from} -> {einops_to}'.replace(' d', ''),
                **einops_dims
            )
            cls_mask = repeat(
                cls_mask, 'b () -> (b r) ()', r=r
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        patch_size = img_size if type(patch_size
                                     ) is tuple else to_2tuple(patch_size)
...
class HeadMLP(nn.Module):
    def __init__(self, n_input, n_classes, n_hidden=512, p=0.1):
        ...
        if n_hidden is None:
            ...
            self.block_forward = nn.Sequential(
                nn.Dropout(p=p), nn.Linear(n_input, n_hidden, bias=True),
                nn.BatchNorm1d(n_hidden), nn.ReLU(inplace=True),
                nn.Dropout(p=p), nn.Linear(n_hidden, n_classes, bias=True)
            )
