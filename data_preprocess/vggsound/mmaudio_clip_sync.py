...
sys.path.append("/mnt/shared-storage-user/xuxuenan/workspace/mmaudio")
...
def get_video_feature(
    ...
    # print("clip: ", clip_features.shape, ", sync: ", sync_features.shape)
    return {"clip": clip_features, "sync": sync_features}


def init_hdf5_datasets(h5f, n_samples):

    clip_shape = (80, 1024)
    sync_shape = (240, 768)
    str_dtype = h5py.string_dtype(encoding="utf-8")

    clip_ds = h5f.create_dataset(
        "clip",
        shape=(n_samples, *clip_shape),
        dtype=np.float32,
    )

    sync_ds = h5f.create_dataset(
        "sync",
        shape=(n_samples, *sync_shape),
        maxshape=(None, *sync_shape),
        dtype=np.float32,
        chunks=(1, *sync_shape),
    )

    aid_ds = h5f.create_dataset(
        "audio_id",
        shape=(n_samples, ),
        dtype=str_dtype,
    )

    return clip_ds, sync_ds, aid_ds
...
def main(
    ...
    if part_idx < 0 or part_idx >= num_parts:
        raise ValueError(
            f"part_idx must be in [0, {num_parts - 1}], got {part_idx}"
        )

    model_config = ModelConfig(
        clip_ckpt="/mnt/brainllm_s3/checkpoints/apple--DFN5B-CLIP-ViT-H-14-378",
        synchformer_ckpt=
        "/mnt/brainllm_s3/checkpoints/hkchengrex--MMAudio/ext_weights/synchformer_state_dict.pth"
    )
    ...
    with open(video_jsonl, "r") as reader, h5py.File(output_h5, "w") as hf:
        clip_ds, clip_len, sync_len = None, 80, 240

        all_lines = reader.readlines()
        if num_parts > 1:
            chunks = np.array_split(all_lines, num_parts)
            lines = chunks[part_idx].tolist()
            print(
                f"[shard] part {part_idx + 1}/{num_parts}: "
                f"{len(lines)} lines (jsonl total {len(all_lines)})"
            )
        else:
            lines = all_lines

        if not lines:
            log.warning("no lines in this part; empty output")
            return

        for idx, line in enumerate(tqdm(lines)):
            row = json.loads(line)
            aid = row["audio_id"]
            features = get_video_feature(row["audio"], feature_utils)
            clip_f = features["clip"][0].cpu().numpy()
            sync_f = features["sync"][0].cpu().numpy()
            if clip_ds is None:
                clip_ds, sync_ds, aid_ds = init_hdf5_datasets(hf, len(lines))

            if clip_f.shape[0] < clip_len:
                clip_f = np.pad(
                    clip_f, ((0, clip_len - clip_f.shape[0]), (0, 0))
                )
            if sync_f.shape[0] < sync_len:
                sync_f = np.pad(
                    sync_f, ((0, sync_len - sync_f.shape[0]), (0, 0))
                )
            clip_ds[idx] = clip_f
            sync_ds[idx] = sync_f
            aid_ds[idx] = aid
