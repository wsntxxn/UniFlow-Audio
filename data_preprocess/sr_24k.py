...
def process_partition(files, input_dir, output_partition_dir, dataset_name):
        ...
        try:
            ...
            if duration > 20:
                ...
                for i in range(num_clips):
                    ...
                    clip_rel_path = os.path.splitext(rel_path)[0] + f"_clip{i+1}" + os.path.splitext(rel_path)[1]
                    raw_target = os.path.abspath(os.path.join(raw_dir, clip_rel_path))
                    low_target = os.path.abspath(os.path.join(low_dir, clip_rel_path))
                    ...
                    low_res_audio = apply_lowpass_filter(clip, sr=24000, filter_type=filter_type, cutoff_freq=cutoff_freq, order=order)
                    ...
                    unique_id = generate_unique_audio_id(posixpath.join(*posixpath.split(clip_rel_path)), dataset_name)
                    
                    # **使用绝对路径**
                    audio_entry = {"audio_id": unique_id, "audio": raw_target}
                    content_entry = {"audio_id": unique_id, "caption": low_target}
                    
                    audio_entries.append(audio_entry)
                    content_entries.append(content_entry)
                    
                    print(f"Processed: {file_path} -> raw: {raw_target} | low: {low_target}")
            else:
                ...
                filter_type, cutoff_freq, order = random_lowpass_filter()
                low_res_audio = apply_lowpass_filter(audio, sr=24000, filter_type=filter_type, cutoff_freq=cutoff_freq, order=order)
                ...
                unique_id = generate_unique_audio_id(posixpath.join(*posixpath.split(rel_path)), dataset_name)
                
                audio_entry = {"audio_id": unique_id, "audio": raw_target_base}
                content_entry = {"audio_id": unique_id, "caption": low_target_base}
                
                audio_entries.append(audio_entry)
                content_entries.append(content_entry)
                
                print(f"Processed: {file_path} -> raw: {raw_target_base} | low: {low_target_base}")
    ...
    print(f"Saved {len(audio_entries)} entries to {audio_jsonl_path} and {content_jsonl_path}")

def main():
    parser = argparse.ArgumentParser(description="Process audio dataset: resample to 24kHz and apply lowpass filter (2-4kHz cutoff). Split input into train/val/test and output to a new directory without modifying the original dataset.")
    parser.add_argument('--input_folder', "-i", type=str, required=True, help='Input folder containing original audio files (can be nested)')
    parser.add_argument('--output_folder', "-o", type=str, required=True, help='Output folder to create new dataset with train/val/test splits')
    parser.add_argument('--dataset_name', "-d", type=str, required=True, help='Dataset name for unique ID generation')
    ...
    # 分割为 train, val, test
    train_files, val_files, test_files = split_files(all_files, train_ratio=args.train_ratio, val_ratio=args.val_ratio)
    print(f"Split into {len(train_files)} train, {len(val_files)} val, {len(test_files)} test files")
    
    for partition, files in zip(["train", "val", "test"], [train_files, val_files, test_files]):
        partition_output = os.path.join(output_folder, partition)
        os.makedirs(partition_output, exist_ok=True)
        print(f"Processing partition {partition} with {len(files)} files, output to {partition_output}")
