...
def segment_videos(
    ...
    Args:
        raw_data_dir: The directory of raw GreatestHits data.
    ...
    if Path(processed_video_record).exists():
...
def extract_clip_features(
    ...
    with File(h5_path, "w") as hf, torch.no_grad():
        for batch in tqdm(
            dataloader, desc="Extracting CLIP features", leave=False
        ):
    ...
    feature_df.to_csv(output_dir / "clip_features.csv", index=False, sep="\t")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="The CLIP model name or path",
    )
    parser.add_argument(
        "--num_workers",
        "-c",
    ...
    video_files = list(raw_data_dir.glob("*_denoised.mp4"))
    ...
    extract_clip_features(
        ...
        args.num_workers,
        args.feature_dir,
        clip_model_name=args.clip_model_name_or_path
    )
