# nohup bash bash_scripts_ali/train_double_content+cross_attn_adapter_init_8gpus.sh > logs/train_double_content+cross_attn_adapter_init_8gpus.log 2>&1  &
...
TORCH_NCCL_TRACE_BUFFER_SIZE=67108864 NCCL_DEBUG=INFO  NCCL_ASYNC_ERROR_HANDLING=1 accelerate launch --config_file configs_ali/accelerate/nvidia/8gpus.yaml train.py \
    ...
    exp_dir=experiments/DummyContentAudioDiffusion/double_content+cross_attn_adapter_init_8gpus \
    trainer.wandb_config.project=DoubleContentAudioDiffusion \
    trainer.wandb_config.name=double_content+cross_attn_adapter_init_8gpus