# nohup bash bash_scripts_ali/train_DummyContentAudioDiffusion_2gpus_tts+tta.sh > logs/train_DummyContentAudioDiffusion_2gpus_tts+tta.log 2>&1  &
...
TORCH_NCCL_TRACE_BUFFER_SIZE=67108864 NCCL_DEBUG=INFO  NCCL_ASYNC_ERROR_HANDLING=1 accelerate launch --config_file configs_ali/accelerate/nvidia/2gpus.yaml train.py \
    ...
    exp_dir=experiments/DummyContentAudioDiffusion/tts+tta_2gpus \
    trainer.wandb_config.project=DummyContentAudioDiffusion \
    trainer.wandb_config.name=tts+tta_2gpus