# nohup bash bash_scripts_ali/train_DummyContentAudioDiffusion_8gpus_tts+tta_resume.sh > logs/train_DummyContentAudioDiffusion_8gpus_tts+tta_resume.log 2>&1  &
...
TORCH_NCCL_TRACE_BUFFER_SIZE=67108864 NCCL_DEBUG=INFO  NCCL_ASYNC_ERROR_HANDLING=1 accelerate launch --config_file configs_ali/accelerate/nvidia/8gpus.yaml train.py \
    ...
    exp_dir=experiments/DummyContentAudioDiffusion/tts+tta_8gpus_resume \
    trainer.wandb_config.project=DummyContentAudioDiffusion \
    trainer.wandb_config.name=tts+tta_8gpus_resume \
    +trainer.resume_from_checkpoint=experiments/DummyContentAudioDiffusion/tts+tta_8gpus/checkpoints/epoch_20
